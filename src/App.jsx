...
function Clientes({ onVerPerfil, clientes = [], onClienteAgregado, onEliminarCliente, onMejorarPlan, user, planActual = "gratis" }) {
  ...
  return (
    <>
      ...
      {tab === "clientes" && (
        <>
          {/* Form agregar — full screen modal */}
          <AnimatePresence>
            {mostrarForm && (
              <motion.div initial={{ opacity: 0, y: 40 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 40 }} transition={{ type: "spring", damping: 28, stiffness: 260 }}
                style={{ position: "fixed", inset: 0, zIndex: 200, display: "flex", flexDirection: "column", background: COLORS.bg }}>
                {/* Header */}
                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "calc(14px + env(safe-area-inset-top)) 20px 16px", borderBottom: `1px solid ${COLORS.border}`, background: COLORS.surface, flexShrink: 0 }}>
                  <div style={{ fontSize: 18, fontWeight: 700, color: COLORS.text }}>Agregar cliente</div>
                  <button onClick={() => { setMostrarForm(false); setClienteEncontrado(null); setUsernameBusq(""); setBusqError(""); setModoAgregar("buscar") }}
                ...
                {/* Content */}
                <div style={{ flex: 1, overflowY: "auto", padding: "20px 16px", scrollbarWidth: "none", paddingBottom: "calc(20px + env(safe-area-inset-bottom))" }}>
                  {/* Tabs */}
                  <div style={{ display: "flex", gap: 8, marginBottom: 20 }}>
                    {[["buscar", "@ Buscar usuario"], ["manual", "+ Agregar manual"]].map(([m, label]) => (
                      <button key={m} onClick={() => { setModoAgregar(m); setClienteEncontrado(null); setUsernameBusq(""); setBusqError("") }}
                        style={{ flex: 1, background: modoAgregar === m ? COLORS.accent : COLORS.surface2, border: `1px solid ${modoAgregar === m ? COLORS.accent : COLORS.border2}`, borderRadius: 12, padding: "11px 0", color: modoAgregar === m ? "#fff" : COLORS.textSub, fontSize: 13, fontWeight: 600, cursor: "pointer" }}>
                        {label}
                      </button>
                    ))}
                  </div>
                  ...
                  {modoAgregar === "buscar" ? (
                    <>
                      ...
                      <div style={{ display: "flex", gap: 8, marginBottom: 12 }}>
                        <button onClick={() => { setMostrarForm(false); setClienteEncontrado(null); setUsernameBusq(""); setBusqError("") }}
                          style={{ flex: 1, background: COLORS.surface2, border: `1px solid ${COLORS.border}`, borderRadius: 12, padding: "13px 0", color: COLORS.textSub, fontSize: 14, cursor: "pointer" }}>Cancelar</button>
                        <motion.button whileTap={{ scale: 0.97 }} onClick={buscarPorUsername} disabled={buscando || !usernameBusq.trim()}
                          style={{ flex: 2, background: COLORS.accent, border: "none", borderRadius: 12, padding: "13px 0", color: "#fff", fontSize: 14, fontWeight: 600, cursor: "pointer", opacity: buscando || !usernameBusq.trim() ? 0.5 : 1 }}>
                      ...
                      {clienteEncontrado === false && !busqError && (
                        <div style={{ fontSize: 13, color: COLORS.textMuted, textAlign: "center", padding: "8px 0", lineHeight: 1.5 }}>
                          No se encontró @{usernameBusq}.<br/>
                          <span style={{ fontSize: 12 }}>Si el usuario existe, puede ser un problema de permisos.</span>
                        </div>
                      )}
                      {clienteEncontrado && clienteEncontrado.trainer_id !== null && (
                        <div style={{ fontSize: 13, color: COLORS.yellow, textAlign: "center", padding: "8px 0" }}>
                          Este usuario ya tiene un entrenador asignado.
                        </div>
                      )}
                      {clienteEncontrado && clienteEncontrado.trainer_id === null && (
                        <>
                          <div style={{ background: COLORS.surface2, borderRadius: 12, padding: "14px 16px", display: "flex", alignItems: "center", gap: 12, marginBottom: 12 }}>
                            {clienteEncontrado.avatar_url
                              ? <img src={clienteEncontrado.avatar_url} style={{ width: 48, height: 48, borderRadius: 8, objectFit: "cover" }} />
                              : <div style={{ width: 48, height: 48, borderRadius: 8, background: COLORS.accent + "33", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 20, fontWeight: 700, color: COLORS.accent }}>
                                  {(clienteEncontrado.nombre || "?")[0].toUpperCase()}
                                </div>
                            }
                            <div>
                              <div style={{ fontSize: 15, fontWeight: 600, color: COLORS.text }}>{clienteEncontrado.nombre || "Sin nombre"}</div>
                              <div style={{ fontSize: 13, color: COLORS.textMuted }}>@{clienteEncontrado.username}</div>
                            </div>
                          </div>
                          <motion.button whileTap={{ scale: 0.97 }} onClick={vincularCliente} disabled={cargando}
                            style={{ background: COLORS.accent, border: "none", borderRadius: 12, padding: "14px 0", color: "#fff", fontSize: 15, fontWeight: 600, cursor: "pointer", width: "100%", opacity: cargando ? 0.5 : 1 }}>
                    ...
                    <>
                      <div style={{ fontSize: 13, color: COLORS.textMuted, marginBottom: 14 }}>El cliente completa sus datos al crear su cuenta.</div>
                      ...
                      <div style={{ display: "flex", gap: 8 }}>
                        <button onClick={() => setMostrarForm(false)} style={{ flex: 1, background: COLORS.surface2, border: `1px solid ${COLORS.border}`, borderRadius: 12, padding: "13px 0", color: COLORS.textSub, fontSize: 14, cursor: "pointer" }}>Cancelar</button>
                        <motion.button whileTap={{ scale: 0.97 }} onClick={agregarCliente} disabled={cargando || !nuevo.nombre.trim()}
                          style={{ flex: 2, background: COLORS.accent, border: "none", borderRadius: 12, padding: "13px 0", color: "#fff", fontSize: 14, fontWeight: 600, cursor: "pointer", opacity: cargando ? 0.5 : 1 }}>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-end" }}>
        <div>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            ...
            {conPrecio.length > 0 && (
              <button onClick={exportarExcel} disabled={exportando} title="Descarga un Excel (.xlsx). Podés subirlo a Google Drive y abrirlo directo con Google Sheets." style={{ background: COLORS.surface, border: `1px solid ${COLORS.border}`, borderRadius: 8, padding: "5px 10px", color: COLORS.textSub, fontSize: 11, fontWeight: 600, cursor: exportando ? "wait" : "pointer", display: "flex", alignItems: "center", gap: 5, opacity: exportando ? 0.6 : 1 }}>
    ...
    <div style={{ background: COLORS.bg, height: "var(--app-height, 100dvh)", display: "flex", fontFamily }}>
      ...
      {!isMobile && (
        <div style={{ width: 76, background: COLORS.bg, borderRight: `1px solid ${COLORS.border}`, display: "flex", flexDirection: "column", height: "var(--app-height, 100dvh)", position: "sticky", top: 0, flexShrink: 0, alignItems: "center" }}>
          ...
          <div style={{ padding: "4px 6px 20px", width: "100%", display: "flex", flexDirection: "column", gap: 1 }}>
            <div style={{ height: 1, background: COLORS.border, margin: "4px 4px 8px" }} />
            <button onClick={() => setPlanesAbierto(true)}
              ...
              <svg width={20} height={20} viewBox="0 0 24 24" fill="none" stroke={COLORS.textMuted} strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round"><polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2"/></svg>
              <span style={{ fontSize: 10, fontWeight: 500, color: COLORS.textMuted }}>Planes</span>
            </button>
            <button onClick={() => setChatbotAbierto(true)}
              style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 3, background: "none", border: "none", cursor: "pointer", width: "100%", padding: "8px 0", borderRadius: 10 }}>
              <svg width={20} height={20} viewBox="0 0 24 24" fill="none" stroke={COLORS.textMuted} strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round"><path d="M12 2C6.477 2 2 6.477 2 12c0 1.89.525 3.66 1.438 5.168L2 22l4.832-1.438A9.956 9.956 0 0012 22c5.523 0 10-4.477 10-10S17.523 2 12 2z"/><path d="M8 10h.01M12 10h.01M16 10h.01"/></svg>
              <span style={{ fontSize: 10, fontWeight: 500, color: COLORS.textMuted }}>FAQ IA</span>
            </button>
            <a href="https://wa.me/541122987419" target="_blank" rel="noopener noreferrer"
              style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 3, textDecoration: "none", width: "100%", padding: "8px 0", borderRadius: 10 }}>
              <svg width={20} height={20} viewBox="0 0 24 24" fill="#25D366"><path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z"/></svg>
              <span style={{ fontSize: 10, fontWeight: 500, color: COLORS.textMuted }}>WhatsApp</span>
            </a>
            <a href="https://instagram.com/tupersonal.fit" target="_blank" rel="noopener noreferrer"
              style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 3, textDecoration: "none", width: "100%", padding: "8px 0", borderRadius: 10 }}>
              <svg width={20} height={20} viewBox="0 0 24 24" fill="none"><defs><linearGradient id="ig-sb" x1="0" y1="1" x2="1" y2="0"><stop offset="0%" stopColor="#f09433"/><stop offset="25%" stopColor="#e6683c"/><stop offset="50%" stopColor="#dc2743"/><stop offset="75%" stopColor="#cc2366"/><stop offset="100%" stopColor="#bc1888"/></linearGradient></defs><rect width="24" height="24" rx="6" fill="url(#ig-sb)"/><circle cx="12" cy="12" r="4" stroke="#fff" strokeWidth="1.8" fill="none"/><circle cx="17.5" cy="6.5" r="1.2" fill="#fff"/><rect x="3" y="3" width="18" height="18" rx="5" stroke="#fff" strokeWidth="1.8" fill="none"/></svg>
              <span style={{ fontSize: 10, fontWeight: 500, color: COLORS.textMuted }}>Instagram</span>
            </a>
            <div style={{ height: 1, background: COLORS.border, margin: "8px 4px 4px" }} />
            <button onClick={onLogout}
              style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 3, background: "none", border: "none", cursor: "pointer", width: "100%", padding: "8px 0", borderRadius: 10 }}>
